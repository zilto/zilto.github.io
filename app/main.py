...
@app.get("/blogpost/", status_code=200, response_class=HTMLResponse)
def blogpost(request: Request):
    # blogpost_html = markdown.markdownFromFile(input=f"/blogs/")
    context = {"request": request}
    return templates.TemplateResponse("blogpost.html", context)


@app.get("/braindump/", status_code=200, response_class=HTMLResponse)
def braindump(request: Request):
    context = {"request": request}
